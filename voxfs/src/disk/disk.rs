...
use crate::{AidFSError, AidFSErrorConvertible, ByteSerializable, OSManager};
...
pub struct Disk<'a, 'b, E: AidFSErrorConvertible> {
...
macro_rules! unwrap_error_aidfs_convertible {
    ($v:expr) => {
        match $v {
            Ok(val) => val,
            Err(e) => return Err(e.into_aidfs_error()),
        }
    };
}

impl<'a, 'b, E: AidFSErrorConvertible> Disk<'a, 'b, E> {
    pub fn make_new_filesystem(
        handler: &'a mut dyn DiskHandler<E>,
        manager: &'b mut dyn OSManager,
    ) -> Result<Self, AidFSError<E>> {
    ...
    pub fn make_new_filesystem_with_root(
        ...
        if block_size % 64 != 0 {
            return Err(AidFSError::InvalidBlockSize);
        }
    ...
    pub fn write_bitmaps(&mut self) -> Result<(), AidFSError<E>> {
    ...
    pub fn store_tag_first_free(&mut self, tag: TagBlock) -> Result<(), AidFSError<E>> {
    ...
    pub fn list_inodes(&self) -> Result<Vec<INode>, AidFSError<E>> {
        ...
        for i in 0..self.tag_bitmap.len() {
            if self.tag_bitmap.bit_at(i).unwrap() {
                ...
                nodes.push(
                    ...
                    {
                        Some(node) => node,
                        None => return Err(AidFSError::CorruptedINode),
                    },
                );
            }
        }
    ...
    fn load_tags(&self) -> Result<Vec<TagBlock>, AidFSError<E>> {
        ...
        for i in 0..self.tag_bitmap.len() {
            if self.tag_bitmap.bit_at(i).unwrap() {
                ...
                tags.push(
                    ...
                    {
                        Some(tag) => tag,
                        None => return Err(AidFSError::CorruptedTag),
                    },
                );
            }
        }
    ...
    pub fn create_new_file_first_free(&mut self, file_inode: INode, contents: Vec<u8>) -> Result<(), AidFSError<E>> {
